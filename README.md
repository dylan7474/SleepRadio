...
**Mixer & timer**
- **Instrument panel** (top of the screen): a steam-age brass‑and‑copper panel over the radio deck —
  a hand‑wheel menu button, riveted nameplate, the cover art in a round porthole, ARTIST/TRACK (or
  STATION/NOW PLAYING…) glass readout windows that scroll long titles, a status lamp, a gauge for progress
  and a slide‑rule seek bar (tap or drag). Every source type shares the same fixed layout; live radio dims
  the gauge, time and slide rule. The metal is bitmap art generated by `tools/keyart/make_steam.py`
- **Channel buttons** (menu): choose *four at a time* (2×2 wide keys, two pages) or *one big button at a
  time* — a single large key you swipe sideways through all eight channels, easiest to see for anyone
  with poor eyesight. Saved and part of Backup & restore
